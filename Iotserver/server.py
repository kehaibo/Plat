...
class Echo(protocol.Protocol):#处理事件程序
	...
	def datahanlder(self,data):
		...
		if CRC16_1(data):#crc检验
			self.UUID= datastr[4:36]	
			self.factory.devices[self.UUID] = self #{'UUID':self}
			...
			if self.devicestatus != '2':
				self.devicestatus = '2'
				self.statusmapinfo = '在线'
				self.execute_query_deviceinfoid(self.UUID,self.devicestatus,self.statusmapinfo)
	...
	def connectionLost(self, reason):
		...
		"""
		if reason.type is ConnectionDone:#该类可判断设备为主动断开
			self.devicestatus = 0
			self.statusmapinfo = '离线'
			self.hreatbeattimeout.cancel()
	...
	def get_stream_type(self,cursor,model_id):
		'''获取数据流类型'''
		query_stream_type= "SELECT decive_productmodelname.product_model,decive_upstream.stream_Type,decive_upstream.id from  decive_productmodelname right join  decive_upstream on decive_upstream.model_name_id={} and decive_productmodelname.id={}".format(model_id,model_id) 
		cursor.execute(query_stream_type)
		model_streamtype=list(cursor.fetchall())
		data_typedict={}
		for stream_type in model_streamtype:
			if stream_type[0] is None:
				model_streamtype.remove(stream_type)
		#print(model_streamtype)
		self.model_struct['model_name'] = model_streamtype[0][0]
		data_typedict[model_streamtype[0][1]] = ''
	...
	def get_point_type(self,cursor,model_streamtype):
		...
		for m in model_streamtype:
			query_point_type= "SELECT decive_data_point.TLV_Type,decive_data_point.data_type,decive_data_point.data_name FROM decive_data_point RIGHT JOIN  decive_upstream_datapoint ON decive_upstream_datapoint.data_point_id = decive_data_point.id and decive_upstream_datapoint.upstream_id='{}' WHERE  decive_data_point.data_type is not null".format(m[2]) 
			cursor.execute(query_point_type)
			point_types=cursor.fetchall()
			point_typelist.append(point_types)
		#log.msg(point_typelist)
	...
	def execute_query_deviceinfoid(self,UUID,status,info):
		_device_info_id = sqlconnect.runInteraction(self.query_deviceinfoid,UUID)
		_device_info_id.addCallback(self.execute_device_status,status,info)		
		_device_info_id.addErrback(self.sql_error)

	def query_deviceinfoid(self,cursor,UUID):
		#获取设备状态表关联的设备信息表主键id
		query_deviceinfoid_sql = "SELECT device_id FROM decive_devicestatus WHERE UUID='{}' limit 1;".format(UUID)
		cursor.execute(query_deviceinfoid_sql)
		deviceinfoid = cursor.fetchall()[0]
		return deviceinfoid[0]

	def execute_device_status(self,deviceinfoid,status,info):
		_device_status=sqlconnect.runInteraction(self.insert_device_status,deviceinfoid,status,info)
		_device_status.addErrback(self.sql_error)

	def insert_device_status(self,cursor,deviceinfoid,status,info):
		'''插入设备状态及信息'''
		currenttime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
		insert_status_sql = "INSERT INTO decive_devicestatus (UUID,status,info,currenttime,device_id) VALUES ('{}','{}','{}','{}','{}');".format(self.UUID,status,info,currenttime,deviceinfoid)
		cursor.execute(insert_status_sql)

	def sql_error(self,err):
		errstr = 'error:{}'.format(err.getErrorMessage())
		log.err(errstr)
		if errstr.find('2006') !='-1':
			try:
...
class EchoFactory(protocol.Factory):
	...
	def __init__(self):
		...
		self.heartbeat_time=120 #心跳超时

	def buildProtocol(self,addr):  #重写该函数，该函数返回protocol的实例

		return Echo(self)

	def rmqdatahandler(self,data):
		'''处理rabbitmq接收到的数据'''
		utf8data = data.decode('utf-8')
		dictdata = json.loads(utf8data)
		client = self.devices.get(dictdata['UUID'])
		hexbytes= bytes.fromhex(dictdata['Message']) #16进制字节流
		client.transport.write(hexbytes)
...
class RabbitMQ(object):
	...
	@staticmethod
	@defer.inlineCallbacks
	def set_channel_receive_from_back(Devfactory):
		"""
		...
		for i in range(10):
			...
			l = task.LoopingCall(RabbitMQ.read_from_mq, queue_object,Devfactory)
			l.start(0.2)
		defer.returnValue(1)

	@staticmethod
	@defer.inlineCallbacks
	def read_from_mq(queue_object,Devfactory):
		"""
		读取接受到的消息队列消息，并且处理
		"""
		...
		if body:
		    Devfactory.rmqdatahandler(body)
		    defer.returnValue(1)
		defer.returnValue(0)
...
if __name__ == '__main__':
	...
	reactor.callLater(0.5,RabbitMQ.set_channel_receive_from_back,Devfactory)
