...
*/
function updateupstream(id){
	//修改数据体信息并保存
	var clsobj = document.getElementById(id);//返回指定id元素对象

	var showname = clsobj.getElementsByClassName('col-xs-3 col-sm-3 showname')[0];//返回属性对象
	var pointname = clsobj.getElementsByClassName('col-xs-3  col-sm-3 pointname')[0];
	var tlvtype  = clsobj.getElementsByClassName('col-xs-3 col-sm-3 streamtlvtype')[0];

	$('#update_streammodal').modal('show');

	$('#updatestream_id').val(showname.getAttribute('data-streamid'));
	$('#updatestream_name').val(pointname.id);
	$('#updatestream_showname').val(showname.id);
	$('#updatestream_TLVType').val(tlvtype.id);
...
function updatedownstream(id){
	//修改数据体信息并保存
	var clsobj = document.getElementById(id);//返回指定id元素对象
	var showname = clsobj.getElementsByClassName('col-xs-3  col-sm-3 downshowname')[0];//返回属性对象
	var pointname = clsobj.getElementsByClassName('col-xs-3  col-sm-3 downpointname')[0];
	var tlvtype  = clsobj.getElementsByClassName('col-xs-3 col-sm-3 downstreamtlvtype')[0];

	$('#update_downstreammodal').modal('show');

	$('#downdatestream_id').val(showname.getAttribute('data-streamid'));
	$('#downdatestream_name').val(pointname.id);
	$('#downdatestream_showname').val(showname.id);
	$('#downdatestream_TLVType').val(tlvtype.id);
...
function selectpoint_forstream(){
	//为数据体保存选中的数据点数据
	var selectedpoint = new Array ;
	$("input[name='pointcheckbox']:checked").each(function(){
		point_name = $(this).val();
		selectedpoint.splice(0,0,point_name);
	})
	return selectedpoint
}

function Synchronous_selection(modal_id,upstreamid,checkboxname,selectedpointcls){
	//已经存在的数据点，复选框同步选中状态
	var modalcls = document.getElementById(modal_id);
	var upstreamcls = document.getElementById(upstreamid);

	var datapointlist = modalcls.getElementsByClassName(checkboxname);
	var shownamelist = upstreamcls.getElementsByClassName(selectedpointcls);

	for (var i=0;i<datapointlist.length;i++)
	{
		var datapointvalue = new String;
		datapointvalue=datapointlist[i].value;
		for (var b=0;b<shownamelist.length;b++)
		{
			var shownamevalue = new String;
			shownamevalue = shownamelist[b].innerHTML;
			if (datapointvalue==shownamevalue) 
			{
				datapointlist[i].checked=true;
			}
		}
...
function update_deviceinfo(name){
	//修改设备信息
	var newinfo	= new Object;
	var elementobj = document.getElementsByClassName(name)[0];
	var Gdevicenameobj = elementobj.getElementsByClassName('device_name')[0];
	var Gdeviceuuifobj = elementobj.getElementsByClassName('device_UUID')[0];
	var Gdevicemodelnameobj = elementobj.getElementsByClassName('device_modelname')[0];
	var Gdevicedescobj = elementobj.getElementsByClassName('device_desc')[0];

	var updatemodalobj = document.getElementById('update_deviceinfomodal');
	var Ndevice_nameobj = updatemodalobj.getElementsByClassName('form-control updatedevice_name')[0];
	var Ndevice_uuidobj = updatemodalobj.getElementsByClassName	('updatedevice_uuid')[0];
	var Ndevice_modelnameobj = updatemodalobj.getElementsByClassName('form-control ProductModel_select')[0];
	var Ndevice_devicedescobj = updatemodalobj.getElementsByClassName('form-control device_dec')[0];

	newinfo['device_name']=Ndevice_nameobj.value=Gdevicenameobj.innerHTML;
	newinfo['device_uuid'] = Ndevice_uuidobj.innerHTML = Gdeviceuuifobj.innerHTML;
	newinfo['device_product']=Ndevice_modelnameobj.value=Gdevicemodelnameobj.innerHTML;
	newinfo['device_dec'] = Ndevice_devicedescobj.innerHTML = Gdevicedescobj.innerHTML;
	
	return	newinfo;
}

function devicestatus_style(){
	//设备状态样式
	var alldevicesstatus =  document.getElementsByClassName('devices_status');
	for(var i=0;i<alldevicesstatus.length;i++)
	{
		if (alldevicesstatus[i].innerText==' 在线')
		{
			var one = alldevicesstatus[i].getElementsByClassName('icon-circle')[0].style.color='#0000FF';
		}
